...
module.exports.ChangeInvestReqStatus = async (
  invest_req_status,
  invest_req_id
) => {
...
module.exports.TransferInvestQuery = async (nominee_id, invest_req_id) => {
  var Query = `update user_invest set ui_transfer =? where ui_id = ?`;
  var data = await query(Query, [nominee_id, invest_req_id]);
  return data;
};

module.exports.AddTerminationData = async (total_payout_amount,total_invest_amount,invest_req_id) => {
  var Query = `update user_invest set ui_terminate_payout_amount =?,ui_terminate_invest_amount=? where ui_id = ?`;
  var data = await query(Query, [total_payout_amount,total_invest_amount,invest_req_id]);
  return data;
};
//------------------------------------
...
module.exports.AddWalletAmount = async (payout_amount, user_id) => {
  var Query = `UPDATE users SET u_wallet = u_wallet + ? WHERE u_id = ?`;
  var data = await query(Query, [payout_amount, user_id]);
  return data;
};

module.exports.AddWalletHistory = async (
  user_id,
  contract_type,
  payout_amount,
  currentdate
) => {
  var Query = `insert into wallet (w_u_id,w_contract_type,w_amount,w_date) values (?,?,?,?)`;
...
module.exports.ChangeWithdrawelStatus = async (
  withdrawel_status,
  withdrawel_id
) => {
...
module.exports.ChangeInvestPatmentStatus = async (
  pay_invest_id,
  pay_invest_status
) => {
...
module.exports.ChangeAdminStatus = async (
  activate_admin_status,
  activate_admin_id
) => {
