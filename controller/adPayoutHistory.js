...
module.exports.EditPayout = async (req, res) => {
    try {
        ...
        let { payout_id, ph_payout_cycle, ph_amount, ph_payout_date, ph_deduction_amount, ph_total_payable } = req.body
        ...
        if (checkpayoutData.length > 0) {
            ...
            if (condition !== '') {
                var EditpayoutData = await model.ChangepayoutDataQuery(condition, payout_id)
            }
            if (EditpayoutData) {
